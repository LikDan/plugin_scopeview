...
[![Build](https://github.com/likdan/plugin_scopeview/workflows/Build/badge.svg)](https://github.com/likdan/plugin_scopeview/actions?query=workflow%3ABuild)

<!-- Plugin description -->
**Scope View** adds dedicated panes to the IntelliJ Project window — one per scope group you define — so you can navigate your project by logical area instead of physical directory structure.

Each pane shows only the files and directories covered by its assigned IntelliJ named scopes. Groups and their scopes are fully configurable under **Settings → Tools → Scope View**: create multiple groups, assign any combination of existing named scopes to each, and reorder them freely. All changes are applied immediately without restarting the IDE.
